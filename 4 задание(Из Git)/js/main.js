	...
	var trigger=0, name, age, sex, Data, education, mathematic, asu, cutting, programming, BD; // обьявляем переменные которые понадобятся в ходе работы
		...
		function test(){ // Функция которая опрашивает пользователя
			...
			} while (!(/^([М]+|)$/.test(sex) || /^([Ж]+|)$/.test(sex) || /^([м]+|)$/.test(sex) || /^([ж]+|)$/.test(sex)));
			do {
				Data = prompt("Полис");
			} while (!(/\d/.test(Data)));
			...
			document.getElementById("BD").setAttribute('disabled', 'disabled');
			writeToPage();
		}
